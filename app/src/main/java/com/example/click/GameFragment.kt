...
class GameFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.currentTime.observe(
            viewLifecycleOwner,
            Observer { time ->
                binding.gameFragmentTvTimer.text = DateUtils.formatElapsedTime(time)
            })
        ...
        viewModel.score.observe(
            viewLifecycleOwner,
            Observer { score -> binding.gameFragmentTvScore.text = score.toString() })

        viewModel.scoreColor.observe(
            viewLifecycleOwner,
            Observer { color -> changeScoreColor(color) })

        viewModel.gameFinished.observe(
            viewLifecycleOwner,
    ...
    private fun changeScoreColor(color: String) {
        if (color == "purple") {
            binding.gameFragmentTvScore.setTextColor(
